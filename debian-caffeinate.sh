#!/bin/bash
# Linux equivalent of macOS `caffeinate -i`. Ctrl-C to release.
#
# Blocks idle-sleep and lid-close suspend until the script is killed.
# Persists across AC/battery transitions. Does NOT block critical-battery
# suspend.
#
# Usage:
#   ./debian-caffeinate.sh                       # block idle-sleep + lid-close suspend
#   ./debian-caffeinate.sh "long ml run"         # custom reason (shows in systemd-inhibit --list)

set -euo pipefail

WHAT="idle:handle-lid-switch"

REASON="${1:-keeping system awake}"

echo "caffeinate: blocking [$WHAT] — reason: $REASON"
echo "caffeinate: screen may still blank (DE-controlled). Press Ctrl-C to release."

exec systemd-inhibit \
  --what="$WHAT" \
  --who="${USER}-caffeinate" \
  --why="$REASON" \
  sleep infinity
